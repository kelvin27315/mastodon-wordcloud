...
class Gensokyo:
    ...
    def get_toots(self):
        ...
        """
        #1日の始まりの時刻(JST)
        last_year = timezone("Asia/Tokyo").localize(dt.datetime((TODAY-relativedelta(years=1)).year,1,1,0,0,0,0))
        today =     timezone("Asia/Tokyo").localize(dt.datetime(TODAY.year, TODAY.month, TODAY.day, 0, 0, 0, 0))
        yesterday = timezone("Asia/Tokyo").localize(dt.datetime(YESTERDAY.year, YESTERDAY.month, YESTERDAY.day, 0, 0, 0, 0))
