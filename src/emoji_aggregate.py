...
class Gensokyo:
    ...
    def get_account_toot(self):
        ...
        last_year = timezone("Asia/Tokyo").localize(
            dt.datetime((dt.date.today() - relativedelta(years=1)).year, 1, 2, 0, 0, 0, 0)
        )
...
def emoji_counter(rank):
    ...
    集計
    """
    emoji_rank = pd.DataFrame(
        {"emoji": list(set([re.search(":[a-zA-Z0-9_-]+:", emoji)[0] for emoji in rank])), "num": 0}
    )
    for emoji in tqdm(rank):
        emoji_rank.iat[emoji_rank[emoji_rank["emoji"] == re.search(":[a-zA-Z0-9_-]+:", emoji)[0]].index[0], 1] += int(
            re.search("[0-9]+回", emoji)[0][:-1]
        )
